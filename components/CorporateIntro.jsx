        ...
        <div className="flex flex-col gap-6">
          <ScrollReveal direction="up" delay="0.05">
            <div>
              ...
              <h2 id="corporate-intro-heading" className="text-2xl sm:text-3xl lg:text-4xl font-bold leading-tight text-white">
                Kurumsal <br />
          ...
          <ScrollReveal direction="up" delay="0.15">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {[
                { title: "Sahne Tasarımı", desc: "Özel 3D modelleme & dekor.", border: "border-blue-500/20" },
                { title: "Görüntü Sistemleri", desc: "Yüksek çözünürlüklü LED & Watchout.", border: "border-purple-500/20" },
                { title: "Teknik Prodüksiyon", desc: "Ses, ışık, truss ve reji yönetimi.", border: "border-emerald-500/20" },
              ].map((item, idx) => (
                <div key={idx} className={`rounded-xl border ${item.border} bg-white/5 p-3 backdrop-blur-sm transition-transform hover:-translate-y-1 hover:bg-white/10`}>
                  <strong className="mb-1 block text-xs sm:text-sm font-semibold text-white">{item.title}</strong>
                  <p className="text-[10px] sm:text-xs leading-relaxed text-slate-400">{item.desc}</p>
                </div>
              ))}
            </div>
          </ScrollReveal>
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-xl border border-white/5 bg-black/20 p-4">
                <ul className="grid grid-cols-1 gap-x-4 gap-y-2 sm:grid-cols-2">
                  {["Akustik ve enerji analizi", "Teknik çizim & 3D plan", "Acil durum planlaması", "Truss yük testleri", "Yedekli sistem altyapısı", "Görüntü optimizasyonu"].map((text, idx) => (
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <div className="flex flex-col gap-3">
                  ...
                  <div className="space-y-0.5">
                    <h4 className="text-base sm:text-lg font-bold text-white">Kurumsal Lansman Sahnesi</h4>
                    <p className="text-xs text-slate-400">İstanbul &bull; 2000+ Katılımcı</p>
                  </div>
                </div>
              </div>
