...
(() => {
  ...
  const ensurePreload = (href) => {
    const existing = Array.from(document.querySelectorAll(preloadSelector))
  ...
  const targetHrefs = ${JSON.stringify(targets)}.length
    ...
    : Array.from(new Set(originals.map(l => l.getAttribute("href")).filter(Boolean)));
