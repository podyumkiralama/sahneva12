...
const DEFAULT_REVIEWS = [
  {
    author: "Ahmet K.",
    body: "Sahne kurulumu tam zamanında yetişti ve ekip tüm detaylarla ilgilendi.",
    ratingValue: "5",
    datePublished: "2024-01-15",
  },
  {
    author: "Merve Y.",
    body: "Ses ve ışık kalitesi beklentimizin üstündeydi, tekrar çalışmak isteriz.",
    ratingValue: "5",
...
const normalizeSlug = (slug) => {
  ...
  const withLeadingSlash = cleaned.startsWith("/")
    ? cleaned
    : `/${cleaned}`;
...
const normalizeShippingDetails = (details) => {
  ...
  return items.map((item) => ({
    "@type": "OfferShippingDetails",
    shippingDestination:
      item.shippingDestination ?? DEFAULT_SHIPPING_DETAILS.shippingDestination,
    deliveryTime: item.deliveryTime ?? DEFAULT_SHIPPING_DETAILS.deliveryTime,
    shippingRate: item.shippingRate ?? DEFAULT_SHIPPING_DETAILS.shippingRate,
  }));
};

export const serviceProducts = [
  {
    // Hizmet sayfalarındaki JsonLd bileşenleri bu veri setinden beslenir.
    // Örn: app/(tr)/sahne-kiralama/page.js içindeki JsonLd bileşeni
    // buildServiceProductSchema ile bu girdiyi okur.
    slug: "/sahne-kiralama",
    locale: "tr-TR",
...
export function buildServiceProductSchema({ slug, locale = "tr-TR" }) {
  ...
  const entry = serviceProducts.find(
    (service) =>
      normalizeSlug(service.slug) === normalizedSlug && service.locale === locale,
  );
  ...
  const productNodes = entry.products.map((product, index) => {
    ...
    const reviews =
      Array.isArray(entry.reviews) && entry.reviews.length > 0
        ? entry.reviews
        : DEFAULT_REVIEWS;
    ...
    const node = {
      ...
      offers: {
        ...
        shippingDetails: normalizeShippingDetails(
          product.shippingDetails ?? entry.shippingDetails,
        ),
      ...
      review: reviews.map((review, reviewIndex) => ({
        "@type": "Review",
        name:
          review.name ?? `${product.name} değerlendirmesi ${reviewIndex + 1}`,
        reviewBody: review.body,
        datePublished: review.datePublished ?? "2024-01-10",
        reviewRating: {
          "@type": "Rating",
          ratingValue: review.ratingValue ?? aggregateRating.ratingValue,
          bestRating: review.bestRating ?? aggregateRating.bestRating,
          worstRating: review.worstRating ?? aggregateRating.worstRating,
        },
        author: {
          "@type": "Person",
          name: review.author ?? "Sahneva Müşteri",
        },
      })),
  ...
  const aggregateOffer =
    prices.length > 0
      ? {
          ...
          availability: "https://schema.org/InStock",
          url: `${pageUrl}#teklif`,
          shippingDetails: normalizeShippingDetails(entry.shippingDetails),
  ...
  const baseOfferNodes = baseOfferEntries
    .map((offer, index) => {
      ...
      return {
        ...
        itemCondition: "https://schema.org/NewCondition",
        shippingDetails: normalizeShippingDetails(
          offer.shippingDetails ?? entry.shippingDetails,
        ),
        hasMerchantReturnPolicy:
  ...
  const serviceNode = {
    ...
    image: toAbsoluteUrl(entry.serviceImage),
