...
export function getCssAssetHrefs() {
  ...
  const allowedSegments = ["/_next/static/css/", "/_next/static/chunks/", "/_chunks/"]; // Turbopack can emit CSS under multiple prefixes

  return Array.from(cssEntries).filter((href) =>
    allowedSegments.some((segment) => href.includes(segment)),
  );
}
