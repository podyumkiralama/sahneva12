...
const BELOW_FOLD_STYLE = {
  ...
  // İçerik boyutuna kabaca bir tahmin vererek CLS'i azalt
...
function CtaButton({
  ...
  const baseClasses =
    "group relative inline-flex items-center justify-center min-w-[180px] min-h-[52px] px-6 py-3 rounded-xl font-bold text-base transition-transform duration-200 hover:scale-[1.02] focus:outline-none focus-visible:ring-4 focus-visible:ring-offset-2";

  const variants = {
    primary:
      "bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-900/20 focus-visible:ring-blue-600",
    secondary:
      "bg-white text-slate-900 border border-slate-200 hover:bg-slate-50 shadow-sm focus-visible:ring-slate-400",
    whatsapp:
      "bg-emerald-600 text-white hover:bg-emerald-700 shadow-lg shadow-emerald-900/20 focus-visible:ring-emerald-600",
  };
...
export default function HomePage() {
  return (
    // Ana içerik landmark'ı geri eklendi
    <main id="main-content" className="flex flex-col gap-0">
          ...
          <MotionWrapper animation="scale" delay={500}>
            ...
            >
              {KEYWORDS.map((word) => (
                <span
                  key={word}
                  className="px-3 py-1 rounded-md bg-white/10 border border-white/10 text-sm text-white font-medium backdrop-blur-sm"
                >
                  {word}
                </span>
              ))}
              ...
              <div className="relative z-10 flex flex-col lg:flex-row items-center gap-8 text-center lg:text-left">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-slate-200 text-lg leading-relaxed">
                    Etkinlik alanınıza en uygun sahne ölçülerini mühendislerimizle
                    planlayın.
      ...
      <section
        style={BELOW_FOLD_STYLE}
        className="py-20 bg-slate-50 relative"
      >
        {/* CSS Grid Pattern Arkaplan */}
        <div
          className="absolute inset-0 opacity-40 pointer-events-none
                     bg-[linear-gradient(#e2e8f0_1px,transparent_1px),linear-gradient(90deg,#e2e8f0_1px,transparent_1px)]
                     bg-[size:20px_20px]
