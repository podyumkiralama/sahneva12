...
const CTA_BASE_CLASS =
  "w-full sm:w-auto min-w-[200px] min-h-[48px] text-center group relative text-white font-bold text-base md:text-lg px-6 py-3 rounded-xl shadow-md hover:shadow-lg transition-transform duration-200 hover:scale-105 border border-white/20 focus-ring";

const CTA_OVERLAY_CLASS =
  "absolute inset-0 rounded-xl bg-gradient-to-r from-white/0 via-white/15 to-white/0 opacity-0 group-hover:opacity-100 transition-opacity duration-300";
...
function SectionHeader({
  ...
  return (
    <div className={`${alignment} mb-12`}>
      ...
      <h2
        id={id}
        className={`text-3xl md:text-4xl font-black ${themeClasses.title} mb-4 leading-tight`}
      >
        {title}
...
function KeywordPills() {
  return (
    <div className="flex flex-wrap justify-center gap-3 mt-4 mb-6 max-w-4xl mx-auto">
      {HERO_KEYWORDS.map(({ text, gradient }) => (
        <span
          key={text}
          className={`text-xs md:text-sm font-semibold px-4 py-2 bg-gradient-to-r ${gradient} text-white rounded-full border border-white/20 shadow-lg`}
        >
          {text}
        </span>
      ))}
...
function HeroBackgroundImage({ alt = HERO_IMAGE_ALT, ariaHidden = false }) {
  return (
    <Image
      ...
      priority
      fetchPriority="high"
      placeholder="blur"      // Görsel daha kaliteli yükleme
      quality={70}            // 50'den yukarı, ama çok da ağır değil
      loading="eager"
      decoding="sync"
          ...
          <div className="container">
            <ScrollReveal>
              <SectionHeader
                ...
                description="Türkiye geneli sahne, podyum, LED ekran kiralama ve ses-ışık sistemleri kurulumu sağlıyoruz. Farklı şehirlerdeki ekibimizle ekran kiralama, sahne kiralama, podyum kiralama ve ses-ışık entegrasyonunu tek elden planlayarak kurumsal etkinliklerinizin akışını kesintisiz kılıyoruz."
              />
            </ScrollReveal>
          </div>
