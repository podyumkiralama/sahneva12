...
/* ================== DİNAMİK BİLEŞENLER ================== */
// Hata veren ssr:false kaldırıldı, client logic bileşen içine taşınmalı.
...
const PACKAGES = [
  { key: "mini", name: "Mini Podyum — 12 m²", layout: { width: 3, depth: 4, area: 12, perimeter: 14 }, includes: ["6 × (1×2 m) panel – toplam 12 m²", "Yükseklik: 40 cm", "Kaymaz kaplama", "Kurulum + söküm"], note: "İç mekân konuşma/mini performanslar için ideal." },
  { key: "orta", name: "Orta Podyum — 24 m²", layout: { width: 4, depth: 6, area: 24, perimeter: 20 }, includes: ["12 × (1×2 m) panel – toplam 24 m²", "Yükseklik: 60 cm", "Kaymaz kaplama, merdiven", "Kurulum + söküm + yerinde dengeleme"], note: "Kurumsal sahneler ve canlı performanslar için." },
  { key: "pro", name: "Pro Podyum — 48 m²", layout: { width: 6, depth: 8, area: 48, perimeter: 28 }, includes: ["24 × (1×2 m) panel – toplam 48 m²", "Yükseklik: 80–100 cm", "Kaymaz kaplama, merdiven, rampa, korkuluk", "Kurulum + söküm + çevre etek/brandalama"], note: "Büyük konser/miting sahneleri için." },
];
...
const FAQ_ITEMS = [
  { q: "Podyum kiralama fiyatları nasıl hesaplanır?", a: "Podyum kiralama fiyatları alan (m²), yükseklik, aksesuarlar (korkuluk, rampa, skört, halı) ve nakliye esas alınarak hesaplanır. Platform: 250 TL/m², Halı: 120 TL/m², Skört: 90 TL/mtül, İstanbul kurulum: 8.000 TL. Detaylı teklif için iletişime geçebilirsiniz." },
  { q: "Kurulum ne kadar sürer?", a: "Standart 24-48 m² podyumlar çoğu mekânda 2-6 saat içinde kurulur. Geniş alanlar ve özel gereksinimler ek süre gerektirebilir. Kurulum süresi podyum büyüklüğüne ve mekan koşullarına göre değişiklik gösterir." },
  { q: "Hangi panelleri kullanıyorsunuz?", a: "1×1m ve 2×1m modüler paneller kullanıyoruz. Düzensiz zeminlerde 1×1m paneller esneklik sağlarken, ana sahnelerde 2×1m paneller hızlı kurulum imkanı sunar. Her iki panel de kaymaz kaplama ve güvenlik standartlarına uygundur." },
  { q: "Halı ve skört zorunlu mu?", a: "Halı ve skört zorunlu değildir; görsel bütünlük ve güvenlik için önerilir. Halı kaymaz özelliktedir ve konfor sağlar, skört ise profesyonel görünüm kazandırır. Fiyatlar opsiyonel olarak ayrı hesaplanır." },
];
    ...
    "@graph": [
      {
        ...
        hasOfferCatalog: {
          ...
          itemListElement: PACKAGES.map(pkg => ({
            "@type": "Offer",
            itemOffered: { "@type": "Service", name: pkg.name, description: pkg.note },
    ...
    <section className="relative flex items-center justify-center overflow-hidden bg-slate-900 pt-20 min-h-[100svh] md:min-h-[80vh]">
      <div className="absolute inset-0">
        <Image
          ...
          alt="Profesyonel podyum kurulumu - Modüler sahne sistemi"
          fill
          priority
          ...
          <div className="bg-white rounded-3xl border-2 border-gray-100 shadow-xl p-8">
            ...
            <div className="mt-6 p-6 bg-blue-50 rounded-2xl border border-blue-200">
              <p className="text-blue-800 text-lg">
                <strong>İstanbul içi nakliye, kurulum ve söküm: 8.000 TL</strong><br />
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {PACKAGES.map((pkg, index) => {
            const base = pkg.layout.area * UNIT_PRICES.platform_m2_week;
            const carpet = pkg.layout.area * UNIT_PRICES.carpet_m2_week;
            const skirt = pkg.layout.perimeter * UNIT_PRICES.skirt_ml_week;
            const totalWithSetup = base + carpet + skirt + UNIT_PRICES.istanbul_setup;

            return (
              <article key={pkg.key} className="group h-full bg-white rounded-3xl border-2 border-gray-100 shadow-xl hover:shadow-2xl overflow-hidden transition-all duration-500 flex flex-col">
                ...
                <div className="p-6 flex-grow">
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-200 mb-4">
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between"><span>Platform:</span><span className="font-semibold">{formatTRY(base)}</span></div>
                      <div className="flex justify-between border-t border-gray-300 pt-2"><span className="font-bold">Toplam (İstanbul):</span><span className="font-bold text-blue-700">{formatTRY(totalWithSetup)}</span></div>
                    </div>
                  </div>
...
// 7. TECHNICAL SPECS SECTION (Restored)
...
// 10. ARTICLES / GUIDES SECTION (Restored - SEO Rich Content)
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <article className="lg:col-span-2 bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden">
            ...
            <div className="p-8 md:p-10 prose prose-lg max-w-none prose-p:text-gray-700 prose-headings:font-bold prose-headings:text-gray-900">
              ...
              <ul>
                <li><strong>Detaylı keşif:</strong> mekan analizi ve zemin değerlendirmesi</li>
                <li><strong>Teknik projelendirme:</strong> yük dağılımı hesapları</li>
                <li><strong>Güvenlik sistemleri:</strong> TS EN standartları ve korkuluklar</li>
              </ul>
            </div>
          </article>
          ...
          <article className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-500">
            <header className="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-6 md:p-8">
              <h3 className="text-xl md:text-2xl font-black">Teknik Entegrasyon ve Kurulum</h3>
            </header>
            <div className="p-6 md:p-8 prose prose-lg">
          ...
          <article className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden hover:shadow-2xl transition-all duration-500">
            <header className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 md:p-8">
              <h3 className="text-xl md:text-2xl font-black">Etkinlik Türlerine Özel Çözümler</h3>
            </header>
            <div className="p-6 md:p-8 prose prose-lg">
              <p><strong>Düğünler:</strong> Şık tasarım, halı kaplama, dekoratif aydınlatma.</p>
              <p><strong>Konserler:</strong> Yüksek dayanıklılık, ses izolasyonu, kablo kanalları.</p>
              <p><strong>Lansmanlar:</strong> Markalı kaplama, rampa ve merdiven, LED entegrasyonu.</p>
            </div>
          </article>
        </div>
      </div>
